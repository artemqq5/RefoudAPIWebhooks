...
class RefundRepositoryTransaction(TransactionDataBase):
    ...
    def _ref_account_by_uid(self, account_uid):
        query = "SELECT * FROM `refunded_accounts` WHERE `account_uid` = %s FOR UPDATE;"
        return self._select_one(query, (account_uid,))

    def _update_status_refunded(self, account_uid, refunded_value, commission):
        query = "UPDATE `refunded_accounts` SET `refund_value` = %s, `commission` = %s, `status` = 'success' WHERE `account_uid` = %s;"
        return self._update(query, (refunded_value, commission, account_uid))
